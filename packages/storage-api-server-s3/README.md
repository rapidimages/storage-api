...
```sh
S3_BUCKET=x S3_SECRET=x S3_KEY=x LOG_PRETTY=1 PORT=5000 npm start
```
...
```sh
ᐅ docker pull rapidimages/storage-api:s3-version
ᐅ docker run --rm --name storage-api -e S3_SECRET=x S3_BUCKET=x S3_KEY=x -p 5000:5000 rapidimages/storage-api:s3-version
```
